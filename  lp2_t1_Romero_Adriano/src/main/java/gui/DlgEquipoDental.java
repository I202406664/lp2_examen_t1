...
import model.DentistaAdrianoRC;
import model.EquipoDentalAdrianoRC;
import model.EspecialidadAdrianoRC;
import util.JPAUtil;
...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void cargarDentistas() {
		...
		try {
			String jpql = "select d from DentistaAdrianoRC d";
			List<DentistaAdrianoRC> lstDentista = manager.createQuery(jpql, DentistaAdrianoRC.class).getResultList();

			for (DentistaAdrianoRC dentista : lstDentista) {
	...
	void listar() {
		...
		String jpql = "select e from EquipoDentalAdrianoRC e ";
		try {
			List<EquipoDentalAdrianoRC> resultados = manager.createQuery(jpql, EquipoDentalAdrianoRC.class)
					.getResultList();
			for (EquipoDentalAdrianoRC equipo : resultados) {

				imprimir("Nro equipo..............: " + equipo.getNroEquipo());
				imprimir("Nombre..................: " + equipo.getNombre());
				imprimir("Costo...................: " + equipo.getPrecio());
				imprimir("Fecha de Adquisicion....: " + equipo.getFechaAdquisicion());
				imprimir("Dentista................: " + equipo.getDentista().getNroConsulta() + " - "
						+ equipo.getDentista().getNombreCompleto());
				imprimir("Correo..................: " + equipo.getDentista().getCorreo());
				imprimir("Especialidad............: " + equipo.getDentista().getIdEspecialidad().getTitulo());
	...
	void adicionar() {
		...
		DentistaAdrianoRC dentista = (DentistaAdrianoRC) cboDentistas.getSelectedItem();

		LocalDate fechaAdquisicion = LocalDate.parse(fecha);
		EquipoDentalAdrianoRC equipo = new EquipoDentalAdrianoRC(ADICIONAR, nombre, precio, fechaAdquisicion, estado,
				dentista);
		EntityManager manager = JPAUtil.getEntityManager();
		try {
	...
	void buscar() {
		...
		try {
			EquipoDentalAdrianoRC equipo = manager.find(EquipoDentalAdrianoRC.class, nroEquipo);

			if (equipo == null) {
				mensajeError("Equipo dental no encontrado");
			} else {
				txtNombre.setText(equipo.getNombre());
				txtCosto.setText(equipo.getPrecio() + "");
	...
	void modificar() {
		...
		try {
			...
			equipo.setPrecio(Double.parseDouble(txtCosto.getText()));
			...
			equipo.setDentista((DentistaAdrianoRC) cboDentistas.getSelectedItem());
			manager.merge(equipo);
			manager.getTransaction().commit();
			
			mensajeInfo("Se actualizo el equipo dental");
			limpiar();
		} finally {
			manager.close();
		}
	...
	void eliminar() {
		...
		try {
			...
			mensajeInfo("Se elimino el equipo dental");
			limpiar();
		} finally {
			manager.close();
		}
